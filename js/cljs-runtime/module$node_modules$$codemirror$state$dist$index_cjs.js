...
desc.sections[i++],ins=desc.sections[i++];if(0>ins)posA+=len,posB+=len;else{let endA=posA,endB=posB,text=Text.empty;for(;;){endA+=len;endB+=ins;ins&&inserted&&(text=text.append(inserted[i-2>>1]));if(individual||i==desc.sections.length||0>desc.sections[i+1])break;len=desc.sections[i++];ins=desc.sections[i++]}f(posA,endA,posB,endB,text);posA=endA;posB=endB}}}function mapSet(setA,setB,before,mkSet=!1){let sections=[];mkSet=mkSet?[]:null;setA=new SectionIter(setA);setB=new SectionIter(setB);for(let posA=
0,posB=0;;)if(-1==setA.ins)posA+=setA.len,setA.next();else if(-1==setB.ins&&posB<posA){var skip$jscomp$0=Math.min(setB.len,posA-posB);setB.forward(skip$jscomp$0);addSection(sections,skip$jscomp$0,-1);posB+=skip$jscomp$0}else if(0<=setB.ins&&(setA.done||posB<posA||posB==posA&&(setB.len<setA.len||setB.len==setA.len&&!before))){for(addSection(sections,setB.ins,-1);posA>posB&&!setA.done&&posA+setA.len<posB+setB.len;)posA+=setA.len,setA.next();posB+=setB.len;setB.next()}else if(0<=setA.ins){skip$jscomp$0=
0;let end=posA+setA.len;for(;;)if(0<=setB.ins&&posB>posA&&posB+setB.len<end)skip$jscomp$0+=setB.ins,posB+=setB.len,setB.next();else if(-1==setB.ins&&posB<end){let skip=Math.min(setB.len,end-posB);skip$jscomp$0+=skip;setB.forward(skip);posB+=skip}else break;addSection(sections,skip$jscomp$0,setA.ins);mkSet&&addInsert(mkSet,sections,setA.text);posA=end;setA.next()}else{if(setA.done&&setB.done)return mkSet?ChangeSet.createSet(sections,mkSet):ChangeDesc.create(sections);throw Error("Mismatched change set lengths");
}}function composeSets(setA,setB,mkSet=!1){let sections=[];mkSet=mkSet?[]:null;setA=new SectionIter(setA);setB=new SectionIter(setB);for(let open=!1;;){if(setA.done&&setB.done)return mkSet?ChangeSet.createSet(sections,mkSet):ChangeDesc.create(sections);if(0==setA.ins)addSection(sections,setA.len,0,open),setA.next();else if(0!=setB.len||setB.done){if(setA.done||setB.done)throw Error("Mismatched change set lengths");let len=Math.min(setA.len2,setB.len),sectionLen=sections.length;if(-1==setA.ins){let insB=
-1==setB.ins?-1:setB.off?0:setB.ins;addSection(sections,len,insB,open);mkSet&&insB&&addInsert(mkSet,sections,setB.text)}else-1==setB.ins?(addSection(sections,setA.off?0:setA.len,len,open),mkSet&&addInsert(mkSet,sections,setA.textBit(len))):(addSection(sections,setA.off?0:setA.len,setB.off?0:setB.ins,open),mkSet&&!setB.off&&addInsert(mkSet,sections,setB.text));open=(setA.ins>len||0<=setB.ins&&setB.len>len)&&(open||sections.length>sectionLen);setA.forward2(len);setB.forward(len)}else addSection(sections,
0,setB.ins,open),mkSet&&addInsert(mkSet,sections,setB.text),setB.next()}}function checkSelection(selection,docLength){for(let range of selection.ranges)if(range.to>docLength)throw new RangeError("Selection points outside of document");}function sameArray(a,b){return a==b||a.length==b.length&&a.every((e,i)=>e===b[i])}function compareArray(a,b,compare){if(a.length!=b.length)return!1;for(let i=0;i<a.length;i++)if(!compare(a[i],b[i]))return!1;return!0}function ensureAll(state,addrs){let changed=!1;for(let addr of addrs)ensureAddr(state,
addr)&1&&(changed=!0);return changed}function dynamicFacetSlot(addresses,facet,providers){function get(state){let values=[];for(let i=0;i<providerAddrs.length;i++){let value=getAddr(state,providerAddrs[i]);if(2==providerTypes[i])for(let val of value)values.push(val);else values.push(value)}return facet.combine(values)}let providerAddrs=providers.map(p=>addresses[p.id]),providerTypes=providers.map(p=>p.type),dynamic=providerAddrs.filter(p=>!(p&1)),idx=addresses[facet.id]>>1;return{create(state){for(let addr of providerAddrs)ensureAddr(state,
addr);state.values[idx]=get(state);return 1},update(state,tr){if(!ensureAll(state,dynamic))return 0;tr=get(state);if(facet.compare(tr,state.values[idx]))return 0;state.values[idx]=tr;return 1},reconfigure(state,oldState){var depChanged=ensureAll(state,providerAddrs);let oldProviders=oldState.config.facets[facet.id];oldState=oldState.facet(facet);if(oldProviders&&!depChanged&&sameArray(providers,oldProviders))return state.values[idx]=oldState,0;depChanged=get(state);if(facet.compare(depChanged,oldState))return state.values[idx]=
oldState,0;state.values[idx]=depChanged;return 1}}}function prec(value){return ext=>new PrecExtension(ext,value)}function flatten(extension,compartments,newCompartments){function inner(ext,prec){var known=seen.get(ext);if(null!=known){if(known<=prec)return;let found=result[known].indexOf(ext);-1<found&&result[known].splice(found,1);ext instanceof CompartmentInstance&&newCompartments.delete(ext.compartment)}seen.set(ext,prec);if(Array.isArray(ext))for(let e of ext)inner(e,prec);else if(ext instanceof
...
if(status&2)return status;state.status[addr]=4;status=state.computeSlot(state,state.config.dynamicSlots[addr]);return state.status[addr]=2|status}function getAddr(state,addr){return addr&1?state.config.staticValues[addr>>1]:state.values[addr>>1]}function mergeTransaction(a,b,sequential){var _a;let mapForB,changes;sequential?(sequential=b.changes,mapForB=ChangeSet.empty(b.changes.length),changes=a.changes.compose(b.changes)):(sequential=b.changes.map(a.changes),mapForB=a.changes.mapDesc(b.changes,
!0),changes=a.changes.compose(sequential));return{changes,selection:b.selection?b.selection.map(mapForB):null===(_a=a.selection)||void 0===_a?void 0:_a.map(sequential),effects:StateEffect.mapEffects(a.effects,sequential).concat(StateEffect.mapEffects(b.effects,mapForB)),annotations:a.annotations.length?a.annotations.concat(b.annotations):b.annotations,scrollIntoView:a.scrollIntoView||b.scrollIntoView}}function resolveTransactionInner(state,spec,docSize){let sel=spec.selection,annotations=asArray(spec.annotations);
spec.userEvent&&(annotations=annotations.concat(Transaction.userEvent.of(spec.userEvent)));return{changes:spec.changes instanceof ChangeSet?spec.changes:ChangeSet.of(spec.changes||[],docSize,state.facet(lineSeparator)),selection:sel&&(sel instanceof EditorSelection?sel:EditorSelection.single(sel.anchor,sel.head)),effects:asArray(spec.effects),annotations,scrollIntoView:!!spec.scrollIntoView}}function resolveTransaction(state,specs,filter){var s=resolveTransactionInner(state,specs.length?specs[0]:
{},state.doc.length);specs.length&&!1===specs[0].filter&&(filter=!1);for(var i=1;i<specs.length;i++){!1===specs[i].filter&&(filter=!1);var seq=!!specs[i].sequential;s=mergeTransaction(s,resolveTransactionInner(state,specs[i],seq?s.changes.newLength:state.doc.length),seq)}state=Transaction.create(state,s.changes,s.selection,s.effects,s.annotations,s.scrollIntoView);if(filter){filter=state.startState;specs=!0;for(JSCompiler_temp of filter.facet(changeFilter)){s=JSCompiler_temp(state);if(!1===s){specs=
!1;break}if(Array.isArray(s))if(!0===specs)specs=s;else a:{i=[];for(let iA=0,iB=0;;){let to;if(iA<specs.length&&(iB==s.length||s[iB]>=specs[iA]))seq=specs[iA++],to=specs[iA++];else if(iB<s.length)seq=s[iB++],to=s[iB++];else{specs=i;break a}!i.length||i[i.length-1]<seq?i.push(seq,to):i[i.length-1]<to&&(i[i.length-1]=to)}}}if(!0!==specs){if(!1===specs){specs=state.changes.invertedDesc;var JSCompiler_temp=ChangeSet.empty(filter.doc.length)}else specs=state.changes.filter(specs),JSCompiler_temp=specs.changes,
specs=specs.filtered.mapDesc(specs.changes).invertedDesc;state=Transaction.create(filter,JSCompiler_temp,state.selection&&state.selection.map(specs),StateEffect.mapEffects(state.effects,specs),state.annotations,state.scrollIntoView)}JSCompiler_temp=filter.facet(transactionFilter);for(specs=JSCompiler_temp.length-1;0<=specs;specs--)state=JSCompiler_temp[specs](state),state=state instanceof Transaction?state:Array.isArray(state)&&1==state.length&&state[0]instanceof Transaction?state[0]:resolveTransaction(filter,
asArray(state),!1);JSCompiler_temp=state}else JSCompiler_temp=state;filter=JSCompiler_temp.startState;state=filter.facet(transactionExtender);specs=JSCompiler_temp;for(s=state.length-1;0<=s;s--)(i=state[s](JSCompiler_temp))&&Object.keys(i).length&&(specs=mergeTransaction(JSCompiler_temp,resolveTransactionInner(filter,i,JSCompiler_temp.changes.newLength),!0));return specs==JSCompiler_temp?JSCompiler_temp:Transaction.create(filter,JSCompiler_temp.changes,JSCompiler_temp.selection,specs.effects,specs.annotations,
specs.scrollIntoView)}function asArray(value){return null==value?none:Array.isArray(value)?value:[value]}function makeCategorizer(wordChars){return char=>{if(!/\S/.test(char))return exports.CharCategory.Space;a:if(wordChar)var JSCompiler_inline_result=wordChar.test(char);else{for(JSCompiler_inline_result=0;JSCompiler_inline_result<char.length;JSCompiler_inline_result++){let ch=char[JSCompiler_inline_result];if(/\w/.test(ch)||""<ch&&(ch.toUpperCase()!=ch.toLowerCase()||nonASCIISingleCaseWordChar.test(ch))){JSCompiler_inline_result=
...
ranges[i];if(0<cmpRange(prev,cur))return ranges.slice().sort(cmpRange);prev=cur}return ranges}function findSharedChunks(a,b,textDiff){let inA=new Map;for(var set$jscomp$0 of a)for(a=0;a<set$jscomp$0.chunk.length;a++)0>=set$jscomp$0.chunk[a].maxPoint&&inA.set(set$jscomp$0.chunk[a],set$jscomp$0.chunkPos[a]);set$jscomp$0=new Set;for(let set of b)for(b=0;b<set.chunk.length;b++)a=inA.get(set.chunk[b]),null==a||(textDiff?textDiff.mapPos(a):a)!=set.chunkPos[b]||(null===textDiff||void 0===textDiff?0:textDiff.touchesRange(a,
a+set.chunk[b].length))||set$jscomp$0.add(set.chunk[b]);return set$jscomp$0}function heapBubble(heap,index){for(let cur=heap[index];;){let childIndex=(index<<1)+1;if(childIndex>=heap.length)break;let child=heap[childIndex];childIndex+1<heap.length&&0<=child.compare(heap[childIndex+1])&&(child=heap[childIndex+1],childIndex++);if(0>cur.compare(child))break;heap[childIndex]=cur;heap[index]=child;index=childIndex}}function compare(a,startA,b,startB,length,comparator){a.goto(startA);b.goto(startB);length=
startB+length;let pos=startB;for(startA=startB-startA;;){startB=a.to+startA-b.to||a.endSide-b.endSide;let end=0>startB?a.to+startA:b.to,clipEnd=Math.min(end,length);a.point||b.point?a.point&&b.point&&(a.point==b.point||a.point.eq(b.point))&&sameValues(a.activeForPoint(a.to+startA),b.activeForPoint(b.to))||comparator.comparePoint(pos,clipEnd,a.point,b.point):clipEnd>pos&&!sameValues(a.active,b.active)&&comparator.compareRange(pos,clipEnd,a.active,b.active);if(end>length)break;pos=end;0>=startB&&a.next();
0<=startB&&b.next()}}function sameValues(a,b){if(a.length!=b.length)return!1;for(let i=0;i<a.length;i++)if(a[i]!=b[i]&&!a[i].eq(b[i]))return!1;return!0}function remove(array,index){for(let i=index,e=array.length-1;i<e;i++)array[i]=array[i+1];array.pop()}function insert(array,index,value){for(let i=array.length-1;i>=index;i--)array[i+1]=array[i];array[index]=value}function findMinIndex(value,array){let found=-1,foundPos=1E9;for(let i=0;i<array.length;i++)0>(array[i]-foundPos||value[i].endSide-value[found].endSide)&&
(found=i,foundPos=array[i]);return found}Object.defineProperty(exports,"__esModule",{value:!0});class Text{constructor(){}lineAt(pos){if(0>pos||pos>this.length)throw new RangeError(`Invalid position ${pos} in document of length ${this.length}`);return this.lineInner(pos,!1,1,0)}line(n){if(1>n||n>this.lines)throw new RangeError(`Invalid line number ${n} in ${this.lines}-line document`);return this.lineInner(n,!0,1,0)}replace(from,to,text){let parts=[];this.decompose(0,from,parts,2);text.length&&text.decompose(0,
...
range.from<prev.to){let from=prev.from;prev=Math.max(range.to,prev.to);i<=mainIndex&&mainIndex--;ranges.splice(--i,2,range.anchor>range.head?EditorSelection.range(prev,from):EditorSelection.range(from,prev))}}return new EditorSelection(ranges,mainIndex)}}let nextID=0;class Facet{constructor(combine,compareInput,compare,isStatic,extensions){this.combine=combine;this.compareInput=compareInput;this.compare=compare;this.isStatic=isStatic;this.extensions=extensions;this.id=nextID++;this.default=combine([])}static define(config=
{}){return new Facet(config.combine||(a=>a),config.compareInput||((a,b)=>a===b),config.compare||(config.combine?(a,b)=>a===b:sameArray),!!config.static,config.enables)}of(value){return new FacetProvider([],this,0,value)}compute(deps,get){if(this.isStatic)throw Error("Can't compute a static facet");return new FacetProvider(deps,this,1,get)}computeN(deps,get){if(this.isStatic)throw Error("Can't compute a static facet");return new FacetProvider(deps,this,2,get)}from(field,get){get||(get=x=>x);return this.compute([field],
state=>get(state.field(field)))}}class FacetProvider{constructor(dependencies,facet,type,value){this.dependencies=dependencies;this.facet=facet;this.type=type;this.value=value;this.id=nextID++}dynamicSlot(addresses){var _a;let getter=this.value,compare=this.facet.compareInput,id=this.id,idx=addresses[id]>>1,multi=2==this.type,depDoc=!1,depSel=!1,depAddrs=[];for(let dep of this.dependencies)"doc"==dep?depDoc=!0:"selection"==dep?depSel=!0:0==((null!==(_a=addresses[dep.id])&&void 0!==_a?_a:1)&1)&&depAddrs.push(addresses[dep.id]);
return{create(state){state.values[idx]=getter(state);return 1},update(state,tr){if(depDoc&&tr.docChanged||depSel&&(tr.docChanged||tr.selection)||ensureAll(state,depAddrs))if(tr=getter(state),multi?!compareArray(tr,state.values[idx],compare):!compare(tr,state.values[idx]))return state.values[idx]=tr,1;return 0},reconfigure:(state,oldState)=>{let newVal=getter(state);var oldAddr=oldState.config.address[id];if(null!=oldAddr&&(oldAddr=getAddr(oldState,oldAddr),this.dependencies.every(dep=>dep instanceof
Facet?oldState.facet(dep)===state.facet(dep):dep instanceof StateField?oldState.field(dep,!1)==state.field(dep,!1):!0)||(multi?compareArray(newVal,oldAddr,compare):compare(newVal,oldAddr))))return state.values[idx]=oldAddr,0;state.values[idx]=newVal;return 1}}}}const initField=Facet.define({static:!0});class StateField{constructor(id,createF,updateF,compareF,spec){this.id=id;this.createF=createF;this.updateF=updateF;this.compareF=compareF;this.spec=spec;this.provides=void 0}static define(config){let field=
new StateField(nextID++,config.create,config.update,config.compare||((a,b)=>a===b),config);config.provide&&(field.provides=config.provide(field));return field}create(state){let init=state.facet(initField).find(i=>i.field==this);return((null===init||void 0===init?void 0:init.create)||this.createF)(state)}slot(addresses){let idx=addresses[this.id]>>1;return{create:state=>{state.values[idx]=this.create(state);return 1},update:(state,tr)=>{let oldVal=state.values[idx];tr=this.updateF(oldVal,tr);if(this.compareF(oldVal,
tr))return 0;state.values[idx]=tr;return 1},reconfigure:(state,oldState)=>{if(null!=oldState.config.address[this.id])return state.values[idx]=oldState.field(this),0;state.values[idx]=this.create(state);return 1}}}init(create){return[this,initField.of({field:this,create})]}get extension(){return this}}const Prec_={lowest:4,low:3,default:2,high:1,highest:0};global={highest:prec(Prec_.highest),high:prec(Prec_.high),default:prec(Prec_.default),low:prec(Prec_.low),lowest:prec(Prec_.lowest)};class PrecExtension{constructor(inner,
prec){this.inner=inner;this.prec=prec}}class Compartment{of(ext){return new CompartmentInstance(this,ext)}reconfigure(content){return Compartment.reconfigure.of({compartment:this,extension:content})}get(state){return state.config.compartments.get(this)}}class CompartmentInstance{constructor(compartment,inner){this.compartment=compartment;this.inner=inner}}class Configuration{constructor(base,compartments,dynamicSlots,address,staticValues,facets){this.base=base;this.compartments=compartments;this.dynamicSlots=
dynamicSlots;this.address=address;this.staticValues=staticValues;this.facets=facets;for(this.statusTemplate=[];this.statusTemplate.length<dynamicSlots.length;)this.statusTemplate.push(0)}staticFacet(facet){let addr=this.address[facet.id];return null==addr?facet.default:this.staticValues[addr>>1]}static resolve(base,compartments,oldState){var fields=[];let facets=Object.create(null),newCompartments=new Map;for(var ext of flatten(base,compartments,newCompartments))ext instanceof StateField?fields.push(ext):
(facets[ext.facet.id]||(facets[ext.facet.id]=[])).push(ext);let address=Object.create(null);compartments=[];ext=[];for(let field of fields)address[field.id]=ext.length<<1,ext.push(a=>field.slot(a));fields=null===oldState||void 0===oldState?void 0:oldState.config.facets;for(let id in facets){let providers=facets[id],facet=providers[0].facet;var oldProviders=fields&&fields[id]||[];if(providers.every(p=>0==p.type))address[facet.id]=compartments.length<<1|1,sameArray(oldProviders,providers)?compartments.push(oldState.facet(facet)):
(oldProviders=facet.combine(providers.map(p=>p.value)),compartments.push(oldState&&facet.compare(oldProviders,oldState.facet(facet))?oldState.facet(facet):oldProviders));else{for(let p of providers)0==p.type?(address[p.id]=compartments.length<<1|1,compartments.push(p.value)):(address[p.id]=ext.length<<1,ext.push(a=>p.dynamicSlot(a)));address[facet.id]=ext.length<<1;ext.push(a=>dynamicFacetSlot(a,facet,providers))}}oldState=ext.map(f=>f(address));return new Configuration(base,newCompartments,oldState,
address,compartments,facets)}}const languageData=Facet.define(),allowMultipleSelections=Facet.define({combine:values=>values.some(v=>v),static:!0}),lineSeparator=Facet.define({combine:values=>values.length?values[0]:void 0,static:!0}),changeFilter=Facet.define(),transactionFilter=Facet.define(),transactionExtender=Facet.define(),readOnly=Facet.define({combine:values=>values.length?values[0]:!1});class Annotation{constructor(type,value){this.type=type;this.value=value}static define(){return new AnnotationType}}
...
this._doc=null;selection&&checkSelection(selection,changes.newLength);annotations.some(a=>a.type==Transaction.time)||(this.annotations=annotations.concat(Transaction.time.of(Date.now())))}static create(startState,changes,selection,effects,annotations,scrollIntoView){return new Transaction(startState,changes,selection,effects,annotations,scrollIntoView)}get newDoc(){return this._doc||(this._doc=this.changes.apply(this.startState.doc))}get newSelection(){return this.selection||this.startState.selection.map(this.changes)}get state(){this._state||
this.startState.applyTransaction(this);return this._state}annotation(type){for(let ann of this.annotations)if(ann.type==type)return ann.value}get docChanged(){return!this.changes.empty}get reconfigured(){return this.startState.config!=this.state.config}isUserEvent(event){let e=this.annotation(Transaction.userEvent);return!(!e||!(e==event||e.length>event.length&&e.slice(0,event.length)==event&&"."==e[event.length]))}}Transaction.time=Annotation.define();Transaction.userEvent=Annotation.define();Transaction.addToHistory=
Annotation.define();Transaction.remote=Annotation.define();const none=[];exports.CharCategory=void 0;(function(CharCategory){CharCategory[CharCategory.Word=0]="Word";CharCategory[CharCategory.Space=1]="Space";CharCategory[CharCategory.Other=2]="Other"})(exports.CharCategory||(exports.CharCategory={}));const nonASCIISingleCaseWordChar=/[\u00df\u0587\u0590-\u05f4\u0600-\u06ff\u3040-\u309f\u30a0-\u30ff\u3400-\u4db5\u4e00-\u9fcc\uac00-\ud7af]/;let wordChar;try{wordChar=/[\p{Alphabetic}\p{Number}_]/u}catch(_){}class EditorState{constructor(config,
doc,selection,values,computeSlot,tr){this.config=config;this.doc=doc;this.selection=selection;this.values=values;this.status=config.statusTemplate.slice();this.computeSlot=computeSlot;tr&&(tr._state=this);for(config=0;config<this.config.dynamicSlots.length;config++)ensureAddr(this,config<<1);this.computeSlot=null}field(field,require=!0){field=this.config.address[field.id];if(null==field){if(require)throw new RangeError("Field is not present in this state");}else return ensureAddr(this,field),getAddr(this,
field)}update(...specs){return resolveTransaction(this,specs,!0)}applyTransaction(tr){let conf=this.config,{base,compartments}=conf;for(var effect of tr.effects)effect.is(Compartment.reconfigure)?(conf&&(compartments=new Map,conf.compartments.forEach((val,key)=>compartments.set(key,val)),conf=null),compartments.set(effect.value.compartment,effect.value.extension)):effect.is(StateEffect.reconfigure)?(conf=null,base=effect.value):effect.is(StateEffect.appendConfig)&&(conf=null,base=asArray(base).concat(effect.value));
conf?effect=tr.startState.values.slice():(conf=Configuration.resolve(base,compartments,this),effect=(new EditorState(conf,this.doc,this.selection,conf.dynamicSlots.map(()=>null),(state,slot)=>slot.reconfigure(state,this),null)).values);new EditorState(conf,tr.newDoc,tr.newSelection,effect,(state,slot)=>slot.update(state,tr),tr)}replaceSelection(text){"string"==typeof text&&(text=this.toText(text));return this.changeByRange(range=>({changes:{from:range.from,to:range.to,insert:text},range:EditorSelection.cursor(range.from+
text.length)}))}changeByRange(f){let sel=this.selection;var result1=f(sel.ranges[0]);let changes=this.changes(result1.changes),ranges=[result1.range];result1=asArray(result1.effects);for(let i=1;i<sel.ranges.length;i++){let result=f(sel.ranges[i]);var newChanges=this.changes(result.changes);let newMapped=newChanges.map(changes);for(let j=0;j<i;j++)ranges[j]=ranges[j].map(newMapped);newChanges=changes.mapDesc(newChanges,!0);ranges.push(result.range.map(newChanges));changes=changes.compose(newMapped);
result1=StateEffect.mapEffects(result1,newMapped).concat(StateEffect.mapEffects(asArray(result.effects),newChanges))}return{changes,selection:EditorSelection.create(ranges,sel.mainIndex),effects:result1}}changes(spec=[]){return spec instanceof ChangeSet?spec:ChangeSet.of(spec,this.doc.length,this.facet(EditorState.lineSeparator))}toText(string){return Text.of(string.split(this.facet(EditorState.lineSeparator)||DefaultSplit))}sliceDoc(from=0,to=this.doc.length){return this.doc.sliceString(from,to,
this.lineBreak)}facet(facet){let addr=this.config.address[facet.id];if(null==addr)return facet.default;ensureAddr(this,addr);return getAddr(this,addr)}toJSON(fields){let result={doc:this.sliceDoc(),selection:this.selection.toJSON()};if(fields)for(let prop in fields){let value=fields[prop];value instanceof StateField&&(result[prop]=value.spec.toJSON(this.field(fields[prop]),this))}return result}static fromJSON(json,config={},fields){if(!json||"string"!=typeof json.doc)throw new RangeError("Invalid JSON representation for EditorState");
let fieldInit=[];if(fields)for(let prop in fields){let field=fields[prop],value=json[prop];fieldInit.push(field.init(state=>field.spec.fromJSON(value,state)))}return EditorState.create({doc:json.doc,selection:EditorSelection.fromJSON(json.selection),extensions:config.extensions?fieldInit.concat([config.extensions]):fieldInit})}static create(config={}){let configuration=Configuration.resolve(config.extensions||[],new Map),doc=config.doc instanceof Text?config.doc:Text.of((config.doc||"").split(configuration.staticFacet(EditorState.lineSeparator)||
...
"\n"}get readOnly(){return this.facet(readOnly)}phrase(phrase,...insert){for(let map of this.facet(EditorState.phrases))if(Object.prototype.hasOwnProperty.call(map,phrase)){phrase=map[phrase];break}insert.length&&(phrase=phrase.replace(/\$(\$|\d*)/g,(m,i)=>{if("$"==i)return"$";i=+(i||1);return!i||i>insert.length?m:insert[i-1]}));return phrase}languageDataAt(name,pos,side=-1){let values=[];for(let provider of this.facet(languageData))for(let result of provider(this,pos,side))Object.prototype.hasOwnProperty.call(result,
name)&&values.push(result[name]);return values}charCategorizer(at){return makeCategorizer(this.languageDataAt("wordChars",at).join(""))}wordAt(pos){let {text,from,length}=this.doc.lineAt(pos),cat=this.charCategorizer(pos),start=pos-from;for(pos-=from;0<start;){var prev=findClusterBreak(text,start,!1);if(cat(text.slice(prev,start))!=exports.CharCategory.Word)break;start=prev}for(;pos<length;){prev=findClusterBreak(text,pos);if(cat(text.slice(pos,prev))!=exports.CharCategory.Word)break;pos=prev}return start==
pos?null:EditorSelection.range(start+from,pos+from)}}EditorState.allowMultipleSelections=allowMultipleSelections;EditorState.tabSize=Facet.define({combine:values=>values.length?values[0]:4});EditorState.lineSeparator=lineSeparator;EditorState.readOnly=readOnly;EditorState.phrases=Facet.define({compare(a,b){let kA=Object.keys(a),kB=Object.keys(b);return kA.length==kB.length&&kA.every(k=>a[k]==b[k])}});EditorState.languageData=languageData;EditorState.changeFilter=changeFilter;EditorState.transactionFilter=
transactionFilter;EditorState.transactionExtender=transactionExtender;Compartment.reconfigure=StateEffect.define();class RangeValue{eq(other){return this==other}range(from,to=from){return Range.create(from,to,this)}}RangeValue.prototype.startSide=RangeValue.prototype.endSide=0;RangeValue.prototype.point=!1;RangeValue.prototype.mapMode=exports.MapMode.TrackDel;class Range{constructor(from,to,value){this.from=from;this.to=to;this.value=value}static create(from,to,value){return new Range(from,to,value)}}
class Chunk{constructor(from,to,value,maxPoint){this.from=from;this.to=to;this.value=value;this.maxPoint=maxPoint}get length(){return this.to[this.to.length-1]}findIndex(pos,side,end,startAt=0){let arr=end?this.to:this.from;for(let lo=startAt,hi=arr.length;;){if(lo==hi)return lo;startAt=lo+hi>>1;let diff=arr[startAt]-pos||(end?this.value[startAt].endSide:this.value[startAt].startSide)-side;if(startAt==lo)return 0<=diff?lo:hi;0<=diff?hi=startAt:lo=startAt+1}}between(offset,from,to,f){for(let i=this.findIndex(from,
-1E9,!0),e=this.findIndex(to,1E9,!1,i);i<e;i++)if(!1===f(this.from[i]+offset,this.to[i]+offset,this.value[i]))return!1}map(offset,changes){let value=[],from=[],to=[],newPos=-1,maxPoint=-1;for(let i=0;i<this.value.length;i++){let val=this.value[i],curFrom=this.from[i]+offset;var curTo=this.to[i]+offset,newFrom=void 0;if(curFrom==curTo){newFrom=changes.mapPos(curFrom,val.startSide,val.mapMode);if(null==newFrom)continue;newFrom=curTo=newFrom;if(val.startSide!=val.endSide&&(curTo=changes.mapPos(curFrom,
val.endSide),curTo<newFrom))continue}else if(newFrom=changes.mapPos(curFrom,val.startSide),curTo=changes.mapPos(curTo,val.endSide),newFrom>curTo||newFrom==curTo&&0<val.startSide&&0>=val.endSide)continue;0>(curTo-newFrom||val.endSide-val.startSide)||(0>newPos&&(newPos=newFrom),val.point&&(maxPoint=Math.max(maxPoint,curTo-newFrom)),value.push(val),from.push(newFrom-newPos),to.push(curTo-newPos))}return{mapped:value.length?new Chunk(from,to,value,maxPoint):null,pos:newPos}}}class RangeSet{constructor(chunkPos,
chunk,nextLayer,maxPoint){this.chunkPos=chunkPos;this.chunk=chunk;this.nextLayer=nextLayer;this.maxPoint=maxPoint}static create(chunkPos,chunk,nextLayer,maxPoint){return new RangeSet(chunkPos,chunk,nextLayer,maxPoint)}get length(){let last=this.chunk.length-1;return 0>last?0:Math.max(this.chunkEnd(last),this.nextLayer.length)}get size(){if(this.isEmpty)return 0;let size=this.nextLayer.size;for(let chunk of this.chunk)size+=chunk.value.length;return size}chunkEnd(index){return this.chunkPos[index]+
this.chunk[index].length}update(updateSpec){let {add=[],sort=!1,filterFrom=0,filterTo=this.length}=updateSpec;updateSpec=updateSpec.filter;if(0==add.length&&!updateSpec)return this;sort&&(add=add.slice().sort(cmpRange));if(this.isEmpty)return add.length?RangeSet.of(add):this;let cur=(new LayerCursor(this,null,-1)).goto(0),i=0,spill=[],builder=new RangeSetBuilder;for(;cur.value||i<add.length;)if(i<add.length&&0<=(cur.from-add[i].from||cur.startSide-add[i].value.startSide)){let range=add[i++];builder.addInner(range.from,
range.to,range.value)||spill.push(range)}else if(1==cur.rangeIndex&&cur.chunkIndex<this.chunk.length&&(i==add.length||this.chunkEnd(cur.chunkIndex)<add[i].from)&&(!updateSpec||filterFrom>this.chunkEnd(cur.chunkIndex)||filterTo<this.chunkPos[cur.chunkIndex])&&builder.addChunk(this.chunkPos[cur.chunkIndex],this.chunk[cur.chunkIndex]))cur.nextChunk();else{if(!updateSpec||filterFrom>cur.to||filterTo<cur.from||updateSpec(cur.from,cur.to,cur.value))builder.addInner(cur.from,cur.to,cur.value)||spill.push(Range.create(cur.from,
cur.to,cur.value));cur.next()}return builder.finishInner(this.nextLayer.isEmpty&&!spill.length?RangeSet.empty:this.nextLayer.update({add:spill,filter:updateSpec,filterFrom,filterTo}))}map(changes){if(changes.empty||this.isEmpty)return this;let chunks=[],chunkPos=[],maxPoint=-1;for(let i=0;i<this.chunk.length;i++){let start=this.chunkPos[i],chunk=this.chunk[i],touch=changes.touchesRange(start,start+chunk.length);if(!1===touch)maxPoint=Math.max(maxPoint,chunk.maxPoint),chunks.push(chunk),chunkPos.push(changes.mapPos(start));
else if(!0===touch){let {mapped,pos}=chunk.map(start,changes);mapped&&(maxPoint=Math.max(maxPoint,mapped.maxPoint),chunks.push(mapped),chunkPos.push(pos))}}changes=this.nextLayer.map(changes);return 0==chunks.length?changes:new RangeSet(chunkPos,chunks,changes||RangeSet.empty,maxPoint)}between(from,to,f){if(!this.isEmpty){for(let i=0;i<this.chunk.length;i++){let start=this.chunkPos[i],chunk=this.chunk[i];if(to>=start&&from<=start+chunk.length&&!1===chunk.between(start,from-start,to-start,f))return}this.nextLayer.between(from,
to,f)}}iter(from=0){return HeapCursor.from([this]).goto(from)}get isEmpty(){return this.nextLayer==this}static iter(sets,from=0){return HeapCursor.from(sets).goto(from)}static compare(oldSets,newSets,textDiff,comparator,minPointSize=-1){oldSets=oldSets.filter(set=>0<set.maxPoint||!set.isEmpty&&set.maxPoint>=minPointSize);newSets=newSets.filter(set=>0<set.maxPoint||!set.isEmpty&&set.maxPoint>=minPointSize);let sharedChunks=findSharedChunks(oldSets,newSets,textDiff),sideA=new SpanCursor(oldSets,sharedChunks,
minPointSize),sideB=new SpanCursor(newSets,sharedChunks,minPointSize);textDiff.iterGaps((fromA,fromB,length)=>compare(sideA,fromA,sideB,fromB,length,comparator));textDiff.empty&&0==textDiff.length&&compare(sideA,0,sideB,0,0,comparator)}static eq(oldSets,newSets,from=0,to){null==to&&(to=1E9);var a=oldSets.filter(set=>!set.isEmpty&&0>newSets.indexOf(set));let b=newSets.filter(set=>!set.isEmpty&&0>oldSets.indexOf(set));if(a.length!=b.length)return!1;if(!a.length)return!0;let sharedChunks=findSharedChunks(a,
b);a=(new SpanCursor(a,sharedChunks,0)).goto(from);for(from=(new SpanCursor(b,sharedChunks,0)).goto(from);;){if(a.to!=from.to||!sameValues(a.active,from.active)||a.point&&(!from.point||!a.point.eq(from.point)))return!1;if(a.to>to)return!0;a.next();from.next()}}static spans(sets,from,to,iterator,minPointSize=-1){sets=(new SpanCursor(sets,null,minPointSize)).goto(from);for(minPointSize=sets.openStart;;){let curTo=Math.min(sets.to,to);sets.point?(iterator.point(from,curTo,sets.point,sets.activeForPoint(sets.to),
minPointSize,sets.pointRank),minPointSize=sets.openEnd(curTo)+(sets.to>curTo?1:0)):curTo>from&&(iterator.span(from,curTo,sets.active,minPointSize),minPointSize=sets.openEnd(curTo));if(sets.to>to)break;from=sets.to;sets.next()}return minPointSize}static of(ranges,sort=!1){let build=new RangeSetBuilder;for(let range of ranges instanceof Range?[ranges]:sort?lazySort(ranges):ranges)build.add(range.from,range.to,range.value);return build.finish()}}RangeSet.empty=new RangeSet([],[],null,-1);RangeSet.empty.nextLayer=
...
0}get endSide(){return this.value?this.value.endSide:0}goto(pos,side=-1E9){this.chunkIndex=this.rangeIndex=0;this.gotoInner(pos,side,!1);return this}gotoInner(pos,side,forward){for(;this.chunkIndex<this.layer.chunk.length;){let next=this.layer.chunk[this.chunkIndex];if(!(this.skip&&this.skip.has(next)||this.layer.chunkEnd(this.chunkIndex)<pos||next.maxPoint<this.minPoint))break;this.chunkIndex++;forward=!1}this.chunkIndex<this.layer.chunk.length&&(pos=this.layer.chunk[this.chunkIndex].findIndex(pos-
...
this.setRangeIndex(this.rangeIndex+1);if(0>this.minPoint||this.value.point&&this.to-this.from>=this.minPoint)break}}setRangeIndex(index){if(index==this.layer.chunk[this.chunkIndex].value.length){this.chunkIndex++;if(this.skip)for(;this.chunkIndex<this.layer.chunk.length&&this.skip.has(this.layer.chunk[this.chunkIndex]);)this.chunkIndex++;this.rangeIndex=0}else this.rangeIndex=index}nextChunk(){this.chunkIndex++;this.rangeIndex=0;this.next()}compare(other){return this.from-other.from||this.startSide-
other.startSide||this.rank-other.rank||this.to-other.to||this.endSide-other.endSide}}class HeapCursor{constructor(heap){this.heap=heap}static from(sets,skip=null,minPoint=-1){let heap=[];for(let i=0;i<sets.length;i++)for(let cur=sets[i];!cur.isEmpty;cur=cur.nextLayer)cur.maxPoint>=minPoint&&heap.push(new LayerCursor(cur,skip,minPoint,i));return 1==heap.length?heap[0]:new HeapCursor(heap)}get startSide(){return this.value?this.value.startSide:0}goto(pos,side=-1E9){for(let cur of this.heap)cur.goto(pos,
...
remove(trackOpen,a)}else if(this.cursor.value)if(this.cursor.from>from){this.to=this.cursor.from;this.endSide=this.cursor.startSide;break}else if(a=this.cursor.value,a.point)if(wasPoint&&this.cursor.to==this.to&&this.cursor.from<this.cursor.to)this.cursor.next();else{this.point=a;this.pointFrom=this.cursor.from;this.pointRank=this.cursor.rank;this.to=this.cursor.to;this.endSide=a.endSide;this.cursor.from<from&&(trackExtra=1);this.cursor.next();this.forward(this.to,this.endSide);break}else this.addActive(trackOpen),
this.cursor.next();else{this.to=this.endSide=1E9;break}}if(trackOpen){for(wasPoint=0;wasPoint<trackOpen.length&&trackOpen[wasPoint]<from;)wasPoint++;this.openStart=wasPoint+trackExtra}}activeForPoint(to){if(!this.active.length)return this.active;let active=[];for(let i=this.active.length-1;0<=i&&!(this.activeRank[i]<this.pointRank);i--)(this.activeTo[i]>to||this.activeTo[i]==to&&this.active[i].endSide>=this.point.endSide)&&active.push(this.active[i]);return active.reverse()}openEnd(to){let open=0;

...
(defn update-editor! [text cursor-pos]
  (let [end (count (some-> @!view .-state .-doc str))]
    (.dispatch @!view #js{:changes #js{:from 0 :to end :insert text}
                            :selection #js{:anchor cursor-pos :head cursor-pos}})))

(j/defn eval-at-cursor [on-result ^:js {:keys [state]}]
  (let [cursor-pos (some-> @!view .-state .-selection .-main .-head)
        code (first (str/split (str (some-> @!view .-state .-doc str)) #" => "))]
...
(defn clear-eval []
  (when (not= "" @last-result)
    ...
    (let [code (-> @!view
                   ...
                   first
                   (str @eval-tail))
          cursor-pos (some-> @!view .-state .-selection .-main .-head)]
